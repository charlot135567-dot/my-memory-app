...
st.markdown(f"""
    ...
    /* 固定右下角史努比圖片 */
    .fixed-bottom-img {{ position: fixed; bottom: 10px; right: 10px; width: 120px; z-index: 99; opacity: 0.9; }}
    </style>
...
with tab_play:
    st.subheader("🎯 隨機挑戰")
    # 動畫載入
    m_data = load_lottieurl("raw.githubusercontent.com")
    if LOTTIE_AVAILABLE and m_data:
        st_lottie(m_data, height=150)
    
    st.write(f"題目： 「 **{w1.get('Definition')}** 」 的正確英文是？")
    ans = st.text_input("輸入答案...", key="play_input")
    if st.button("提交"):
        if ans.lower().strip() == str(w1.get("Vocab")).lower().strip():
            st.balloons()
            st.session_state.score += 10
            st.rerun()
        else:
            st.session_state.lives -= 1
            st.error(f"生命值 -1。正確答案是: {w1.get('Vocab')}")
